...
const Account = () => {
    ...
    const { user, logout, isLogin, isLoginModalOpen, setLoginModalOpen } = useContext(UserContext);
    ...
    useEffect(() => {
        // Redirect to login modal if the user is not logged in
        if (!isLogin) {
            ...
            {!isLogin ? (
                ...
                <div className="min-w-[320px] md:max-w-screen-sm lg:max-w-screen-md xl:max-w-screen-lg 2xl:max-w-screen-xl mx-auto mt-32 h-full bg-gray-100 p-4 rounded-lg">
                    ...
                    <div className="w-full flex gap-4">
                        <div className="w-full lg:w-80 flex flex-col p-4 gap-8">
                            <div className="w-full h-24 lg:h-56 flex lg:flex-col lg:justify-between md:flex-row items-center bg-white rounded-lg">
                                <Image
                                    ...
                                    height="0"
                                    sizes="100vw"
                                    className=" md:h-8 w-8 lg:h-24 lg:w-24 mt-2 mx-4 rounded-full"
                            ...
                            <ul className="font-light flex flex-col md:flex-row lg:flex-col gap-2 lg:gap-4">
                                {menuItems.map((item) => (
                                    <li
                                        ...
                                        className="w-full h-12 bg-white rounded-md flex gap-1 lg:gap-4 cursor-pointer p-2 lg:px-4 lg:py-2 text-center"
                                    >
                                        {item.icon}
                        ...
                        <div className="hidden lg:block grow py-4">{getActiveComponent()}</div>
                    </div>
                </div>
            )}
        </div>
